...
def perform_color_and_gradient_thresholding(undisorted_image):
    combined_color_threshold = perform_sobel_magnitude_directional_thresholding(undisorted_image)
    combined_gradient_threshold = perform_color_thresholding(undisorted_image)

    combined_color_and_gradient_threshold = np.zeros_like(combined_color_threshold)
    combined_color_and_gradient_threshold[(combined_color_threshold == 1) and (combined_gradient_threshold == 1)] == 1

    return combined_color_and_gradient_threshold

def perform_color_thresholding(image):
    red = red_thresold(image, red_thresh)
    ...
    combined[(red == 1) and (saturation == 1) and (lightness == 1)] = 1

    return combined

def red_thresold(image, red_thresh):
    red_channel = image[:,:,0]
...
def saturation_threshold(image, s_thresh):
    ...
    hls = cv2.cv2Color(image, cv2.COLOR_RGB2HLS)
...
def lightness_threshold(image, l_thresh):
    ...
    lightness = hls[:,:,1]
    lightness_saturation = np.zeros_like(saturation)
    lightness_saturation[(saturation >= lightness_thresh[0]) & (saturation <= lightness_thresh[1])] = 1
    return lightness_saturation
